...
#define HAPTIC_PIN 42  // active-high haptic module / MOSFET gate
...
enum OpType : uint8_t { OP_CLEAR = 0, OP_RECT = 1, OP_FILL_RECT = 2, OP_LINE = 3, OP_TEXT = 4, OP_BAR_OUTLINE = 5, OP_BAR_FILL = 6, OP_URGENT_BORDER = 7, OP_REISSUE_BARS = 8, OP_CROSS = 9, OP_PROGRESS_META = 10, OP_SCHEDULE_PROGRESS_META = 11, OP_DOTTED_RECT = 12, OP_URGENT_TAB = 13, OP_DOTTED_LINE = 14, OP_BITMAP_REMOTE = 15, OP_TEXT_BITMAP_REMOTE = 16 };
...
BitmapSlot bitmapSlots[MAX_BITMAP_SLOTS];
int bitmapSlotCount = 0;

void clearBitmapSlots() {
  for (int i = 0; i < bitmapSlotCount; i++) {
    if (bitmapSlots[i].black) { free(bitmapSlots[i].black); bitmapSlots[i].black = nullptr; }
    if (bitmapSlots[i].red) { free(bitmapSlots[i].red); bitmapSlots[i].red = nullptr; }
    bitmapSlots[i].loaded = false;
  }
  bitmapSlotCount = 0;
}
...
const int MAX_WALL_EVENTS = 20;
const int MAX_FIRED_KEYS = 48;
WallEvent wallEvents[MAX_WALL_EVENTS];
int wallEventCount = 0;
char firedWallEventKeys[MAX_FIRED_KEYS][97];
int firedWallEventKeyCount = 0;
unsigned long latestWdAlertRevision = 0;
unsigned long lastAckedWdAlertRevision = 0;
bool activeWallAlarm = false;
WallEvent activeWallAlarmEvent;
...
void parseWallEvents(JsonArray arr);
void processWallEvents();
...
bool firedKeySeen(const char* key) {
  if (!key || !key[0]) return false;
  for (int i = 0; i < firedWallEventKeyCount; i++) {
    if (strcmp(firedWallEventKeys[i], key) == 0) return true;
  }
  return false;
}

void rememberFiredKey(const char* key) {
  if (!key || !key[0] || firedKeySeen(key)) return;
  int slot = firedWallEventKeyCount % MAX_FIRED_KEYS;
  strncpy(firedWallEventKeys[slot], key, 96);
  firedWallEventKeys[slot][96] = '\0';
  if (firedWallEventKeyCount < MAX_FIRED_KEYS) firedWallEventKeyCount++;
}

void parseWallEvents(JsonArray arr) {
  wallEventCount = 0;
  for (JsonObject obj : arr) {
    if (wallEventCount >= MAX_WALL_EVENTS) break;
    WallEvent &ev = wallEvents[wallEventCount++];
    memset(&ev, 0, sizeof(WallEvent));
    const char* key = obj["key"] | "";
    const char* type = obj["type"] | "";
    const char* title = obj["title"] | "";
    const char* loc = obj["location"] | "";
    const char* due = obj["due_text"] | "";
    strncpy(ev.key, key, 96); ev.key[96] = '\0';
    strncpy(ev.type, type, 9); ev.type[9] = '\0';
    strncpy(ev.title, title, 96); ev.title[96] = '\0';
    strncpy(ev.location, loc, 72); ev.location[72] = '\0';
    strncpy(ev.dueText, due, 39); ev.dueText[39] = '\0';
    ev.epoch = obj["epoch"] | 0;
    ev.popUp = obj["pop_up"] | false;
    ev.fired = firedKeySeen(ev.key);
    JsonArray pat = obj["pattern_ms"].as<JsonArray>();
    ev.patternLen = 0;
    if (!pat.isNull()) {
      for (JsonVariant v : pat) {
        if (ev.patternLen >= 16) break;
        int ms = v | 0;
        if (ms < 0) ms = 0;
        if (ms > 5000) ms = 5000;
        ev.pattern[ev.patternLen++] = (uint16_t)ms;
      }
    }
  }
  Serial.print("WD events loaded: "); Serial.println(wallEventCount);
}
...
void drawWrappedAlarmText(const String &txt, int x, int y, int maxChars, int lineStep, int maxLines, bool bold, int textSize) {
  display.setFont(bold ? &FreeMonoBold9pt7b : &FreeMono9pt7b);
  ...
  while (remaining.length() > 0 && line < maxLines) {
    ...
    if (part.length() > maxChars) {
      int cut = maxChars;
      for (int i = maxChars; i > 8; i--) {
        if (part.charAt(i) == ' ') { cut = i; break; }
      }
    ...
    display.setCursor(x, y + line * lineStep);
    display.print(part);
    line++;
  }
}
...
bool drawWallAlarmScreen(const WallEvent &ev) {
  ...
  do {
    ...
    display.fillRect(0, 0, 800, 82, GxEPD_BLACK);
    ...
    display.setCursor(34, 126);
    display.print(ev.dueText[0] ? ev.dueText : "DUE NOW");
    drawWrappedAlarmText(String(ev.title[0] ? ev.title : "Task"), 34, 190, 22, 52, 4, true, 3);
    if (ev.location[0]) drawWrappedAlarmText(String(ev.location), 34, 410, 46, 30, 2, false, 2);
    display.drawRect(8, 92, 784, 380, GxEPD_BLACK);
    display.drawRect(12, 96, 776, 372, GxEPD_BLACK);
...
void processWallEvents() {
  ...
  if (millis() - lastWallEventCheck < wallEventCheckIntervalMs) return;
  lastWallEventCheck = millis();
  time_t nowT; time(&nowT);
  unsigned long nowEpoch = (unsigned long)nowT;
  for (int i = 0; i < wallEventCount; i++) {
    WallEvent &ev = wallEvents[i];
    if (!ev.fired && ev.epoch > 0 && nowEpoch >= ev.epoch) {
      fireWallEvent(ev);
      break;
    }
  }
...
bool ackWallAlertRevision(unsigned long rev) {
  if (rev == 0 || rev == lastAckedWdAlertRevision) return true;
  String url = String(relayBaseUrl) + "/api/wd/ack_alert?token=" + relayToken + "&alert_revision=" + String(rev);
  if (httpPOSTempty(url)) {
    lastAckedWdAlertRevision = rev;
    return true;
  }
  return false;
}
...
int loadRemoteBitmapSlot(String endpoint, uint32_t itemId, int targetW, int targetH) {
  targetW = constrain(targetW, 1, SCRIBBLE_MAX_W);
  targetH = constrain(targetH, 1, SCRIBBLE_MAX_H);
  if (bitmapSlotCount >= MAX_BITMAP_SLOTS) { setFault("memory", "bitmap_slots_full"); return -1; }

  size_t maskBytes = ((size_t)targetW * (size_t)targetH + 7) / 8;
  size_t maxLen = 8 + maskBytes * 2;
  uint8_t *tmp = (uint8_t*)heap_caps_malloc(maxLen, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!tmp) tmp = (uint8_t*)malloc(maxLen);
  if (!tmp) { setFault("memory", "bitmap_tmp_alloc"); return -1; }

  String url = String(relayBaseUrl) + endpoint + "?token=" + relayToken + "&id=" + String(itemId) + "&w=" + String(targetW) + "&h=" + String(targetH);
  size_t got = 0;
  bool ok = httpGETBinary(url, tmp, maxLen, got);
  if (!ok) { free(tmp); return -1; }
  if (got < 8 || tmp[0] != 'S' || tmp[1] != 'C' || tmp[2] != 'B' || tmp[3] != '1') {
    free(tmp); setFault("bitmap", "bad_header"); return -1;
  }
  int w = tmp[4] | (tmp[5] << 8);
  int h = tmp[6] | (tmp[7] << 8);
  if (w < 1 || h < 1 || w > SCRIBBLE_MAX_W || h > SCRIBBLE_MAX_H) {
    free(tmp); setFault("bitmap", "bad_dims"); return -1;
  }
  size_t bytes = ((size_t)w * (size_t)h + 7) / 8;
  if (got != 8 + bytes * 2 || bytes > SCRIBBLE_MAX_BYTES) {
    free(tmp); setFault("bitmap", "bad_size"); return -1;
  }

  uint8_t *black = (uint8_t*)heap_caps_malloc(bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  uint8_t *red = (uint8_t*)heap_caps_malloc(bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!black) black = (uint8_t*)malloc(bytes);
  if (!red) red = (uint8_t*)malloc(bytes);
  if (!black || !red) {
    if (black) free(black);
    if (red) free(red);
    free(tmp);
    setFault("memory", "bitmap_slot_alloc");
    return -1;
  }

  memcpy(black, tmp + 8, bytes);
  memcpy(red, tmp + 8 + bytes, bytes);
  free(tmp);

  int slot = bitmapSlotCount++;
  bitmapSlots[slot].loaded = true;
  bitmapSlots[slot].id = itemId;
  bitmapSlots[slot].w = w;
  bitmapSlots[slot].h = h;
  bitmapSlots[slot].black = black;
  bitmapSlots[slot].red = red;
  Serial.print("BITMAP SLOT LOADED: "); Serial.print(slot); Serial.print(" id="); Serial.print(itemId); Serial.print(" "); Serial.print(w); Serial.print("x"); Serial.println(h);
  return slot;
}
...
bool fetchRenderJobNow(unsigned long jobId) {
  ...
  for (JsonObject op : ops) {
    ...
    if (opName == "clear") ro.type = OP_CLEAR;
      ...
      ro.value = -1;
      if (inkId > 0) ro.value = loadRemoteBitmapSlot("/api/scribble_bitmap", inkId, ro.w, ro.h);
    }
    else if (opName == "text_bitmap_remote") {
      ro.type = OP_TEXT_BITMAP_REMOTE;
      uint32_t textId = op["id"] | 0;
      ro.value = -1;
      if (textId > 0) ro.value = loadRemoteBitmapSlot("/api/text_bitmap", textId, ro.w, ro.h);
    }
...
void drawBitmapSlot(int slot, int16_t x0, int16_t y0, int16_t w, int16_t h) {
  if (slot < 0 || slot >= bitmapSlotCount || !bitmapSlots[slot].loaded || !bitmapSlots[slot].black || !bitmapSlots[slot].red) {
    display.setFont();
    display.setTextColor(GxEPD_BLACK);
    display.setCursor(x0 + 4, y0 + 12);
    display.print("bitmap not loaded");
    return;
  }
  BitmapSlot &bs = bitmapSlots[slot];
  int drawW = min((int)w, (int)bs.w);
  int drawH = min((int)h, (int)bs.h);
  int totalW = bs.w;
  for (int yy = 0; yy < drawH; yy++) {
    int rowBase = yy * totalW;
    for (int xx = 0; xx < drawW; xx++) {
      ...
      uint8_t mask = (0x80 >> (bitIndex % 8));
      size_t bi = bitIndex / 8;
      if (bs.red[bi] & mask) display.drawPixel(x0 + xx, y0 + yy, GxEPD_RED);
      else if (bs.black[bi] & mask) display.drawPixel(x0 + xx, y0 + yy, GxEPD_BLACK);
    }
  }
}
  ...
  for (int i = 0; i < renderOpCount; i++) {
    ...
    switch (ro.type) {
      ...
      case OP_BITMAP_REMOTE:
        drawBitmapSlot(ro.value, ro.x, ro.y, ro.w, ro.h);
        break;
      case OP_TEXT_BITMAP_REMOTE:
        drawBitmapSlot(ro.value, ro.x, ro.y, ro.w, ro.h);
        break;
      default: break;
    }
  ...
  if (digitalRead(REFRESH_BUTTON) == LOW) {
    if (millis() - lastButtonAction > 1200) {
      if (activeWallAlarm) {
        ...
        Serial.println("WD alarm dismissed by button");
        if (renderOpCount > 0) deviceState = STATE_RENDER_JOB;
        else deviceState = STATE_POLL_META;
      } else {
        renderJobQueued = true;
      }
  ...
  switch (deviceState) {
    ...
    case STATE_POLL_META:
      ...
      if (fetchRelayMetaNow(latestMeta)) {
        ...
        if (latestWdAlertRevision > 0) ackWallAlertRevision(latestWdAlertRevision);
