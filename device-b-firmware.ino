...
#define BUILD_VERSION "DEVICE_B_CLEAN_SCHEDULER_V3_HTTPS_PSRAM_DIAG_NO_OTA"
...
const char* firmwareVersion = "clean_scheduler_v3_https_psram_diag_no_ota";

// ISRG Root X1 trust anchor. Render provides managed TLS for onrender.com;
// this avoids relying on setInsecure() for the ESP32 HTTPS path.
...
bool configureSecureClient(WiFiClientSecure &client, bool useCa) {
  if (useCa) {
    client.setCACert(ISRG_ROOT_X1);
    Serial.println("TLS mode: CA = ISRG Root X1");
  } else {
    client.setInsecure();
    Serial.println("TLS mode: INSECURE FALLBACK");
  }
...
bool httpsGETAttempt(String url, String &out, bool useCa) {
  WiFiClientSecure client;
  configureSecureClient(client, useCa);
  ...
  if (!http.begin(client, url)) {
    setFault("connection", useCa ? "http_begin_failed_ca" : "http_begin_failed_insecure");
  ...
  if (code != 200) {
    setFault("connection", String(useCa ? "https_ca_" : "https_insecure_") + String(code));
    http.end();
    return false;
  }
...
bool httpGET(String url, String &out) {
  ...
  Serial.println("HTTPS attempt 1: CA verified");
  if (httpsGETAttempt(url, out, true)) return true;

  // Diagnostic fallback: if CA validation/chain handling is the blocker, this should succeed.
  // If both CA and insecure fail, the issue is likely heap/TLS transport/core rather than certificate validation.
  Serial.println("HTTPS attempt 2: insecure fallback after CA failure");
  printMemoryDiagnostics("before insecure fallback");
  if (httpsGETAttempt(url, out, false)) return true;

  return false;
}

bool httpsPOSTAttempt(String url, bool useCa) {
  WiFiClientSecure client;
  configureSecureClient(client, useCa);
  HTTPClient http;
  http.setTimeout(20000);
  http.setReuse(false);
  if (!http.begin(client, url)) {
    setFault("connection", useCa ? "http_post_begin_failed_ca" : "http_post_begin_failed_insecure");
    return false;
  }
  int code = http.POST("");
  lastHttpCode = code;
  ...
  if (!(code >= 200 && code < 300)) {
    setFault("connection", String(useCa ? "https_post_ca_" : "https_post_insecure_") + String(code));
    return false;
  }
  return true;
}
...
bool httpPOSTempty(String url) {
  ...
  Serial.println("HTTPS POST attempt 1: CA verified");
  if (httpsPOSTAttempt(url, true)) return true;

  Serial.println("HTTPS POST attempt 2: insecure fallback after CA failure");
  printMemoryDiagnostics("before POST insecure fallback");
  if (httpsPOSTAttempt(url, false)) return true;
