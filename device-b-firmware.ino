...
// Plain HTTP diagnostic build: WiFiClientSecure intentionally not used.
...
#define BUILD_VERSION "DEVICE_B_CLEAN_SCHEDULER_V1_PLAIN_HTTP_DIAGNOSTIC"
...
const char* relayBaseUrl = "http://device-b-relay.onrender.com";
...
bool runPlainHttpPreflight(const String &host) {
  Serial.println("=== PLAIN HTTP PREFLIGHT START ===");
  ...
  if (!dnsOk) { setFault("dns", "hostByName_failed"); Serial.println("=== PLAIN HTTP PREFLIGHT END ==="); return false; }
  Serial.print("Resolved IP: "); Serial.println(resolvedIP);

  WiFiClient tcp;
  Serial.print("TCP connect to "); Serial.print(host); Serial.println(":80");
  bool tcpOk = tcp.connect(host.c_str(), 80);
  Serial.print("TCP80 OK: "); Serial.println(tcpOk ? "YES" : "NO");
  if (!tcpOk) { setFault("tcp", "connect_80_failed"); Serial.println("=== PLAIN HTTP PREFLIGHT END ==="); return false; }
  tcp.stop();
  Serial.println("=== PLAIN HTTP PREFLIGHT END ===");
  return true;
}
...
bool httpGET(String url, String &out) {
  ...
  if (!runPlainHttpPreflight("device-b-relay.onrender.com")) return false;

  WiFiClient client;
  HTTPClient http;
  http.setFollowRedirects(HTTPC_DISABLE_FOLLOW_REDIRECTS);
  Serial.println("Plain HTTP client configured");
  ...
  if (code != 200) {
    ...
    if (location.length() > 0) { Serial.print("Redirect Location: "); Serial.println(location); }
    // For this diagnostic build, a 301/302/308 proves plain HTTP reached Render.
    // It will still be treated as a fetch failure because following HTTPS would reintroduce TLS.
...
bool httpPOSTempty(String url) {
  ...
  HTTPClient http;
  http.setFollowRedirects(HTTPC_DISABLE_FOLLOW_REDIRECTS);
  Serial.println("Plain HTTP POST client configured");
...
void setup() {
  ...
  delay(400);
  Serial.println();
  Serial.println("BOOT: DEVICE_B_CLEAN_SCHEDULER_V1_PLAIN_HTTP_DIAGNOSTIC");
