...
#define BUILD_VERSION "DEVICE_B_CLEAN_SCHEDULER_V2_STABLE_HTTPS_CA_NO_OTA"
...
const char* firmwareVersion = "clean_scheduler_v2_https_ca_no_ota";
...
void printTlsContext() {
  Serial.print("Free heap before TLS: "); Serial.println(ESP.getFreeHeap());
  Serial.print("Free PSRAM before TLS: "); Serial.println(ESP.getFreePsram());
...
bool httpGET(String url, String &out) {
  ...
  Serial.println("TLS client configured with ISRG Root X1 CA");
  ...
  if (code != 200) {
    setFault("connection", String("https_") + String(code));
    http.end();
    return false;
  }
...
bool httpPOSTempty(String url) {
  ...
  WiFiClientSecure client;
  configureSecureClient(client);
  Serial.println("TLS POST client configured with ISRG Root X1 CA");
  ...
  if (code < 0) {
    Serial.print("HTTP POST error text: "); Serial.println(http.errorToString(code));
  }
  http.end();
  if (!(code >= 200 && code < 300)) {
    setFault("connection", String("https_post_") + String(code));
    return false;
  }
  return true;
}
